...
// main screen - tablets /portrait/landscape? - is there any need?

// there seems to be a bug where the network core stops responding
// and the widget needs a restart. This happens when the system restarts
// and the widget compares the current time with a stored time taken at the 
// sleep point and if they are drastically different it restarts the widget
// Sometimes it seems to just stop after a screen lock and the
// check is not brought into operation.
// Another better check might be to have an additional loop and note if
// the values have remained at zero for a period of time, if so restart
//
// the restart should not display a larger unscaled version of the widget  

// add a test to see if nothing has happened for a while 
...
function widgetOnLoad() {
    ...
    gaugeFace.visible = true;         // setting the whole layer visibility to false and only disclosing it here
                                      // is the only way for a normal resize without a faulty full-size display
...
//======================================================================================
// Function to set the nominal maximum bandwidth
//======================================================================================
function setMaxBandwidth() {       
    NetMonitorCore1.MaxNetIn = preferencesBandwidthValue.text;
    print("NetMonitorCore1.MaxNetIn"+ NetMonitorCore1.MaxNetIn);
}
//=====================
//End function
//=====================
...
function getPrefs(){
    ...
    preferencesBandwidthValue.text      =  Getinivalue(widgetpath+"prefs.ini","prefs","preferencesBandwidthValue","4000");
    preferencesAutoRestartValue.text      =  Getinivalue(widgetpath+"prefs.ini","prefs","preferencesAutoRestartValue","enabled");
...
function savePrefs()
{
         ...
         Setinivalue(widgetpath+"prefs.ini","prefs","preferencesBandwidthValue",preferencesBandwidthValue.text);
         Setinivalue(widgetpath+"prefs.ini","prefs","preferencesAutoRestartValue",preferencesAutoRestartValue.text);
...
function menuitem4OnClick(Sender)
{
          ...
          {
              OpenURL("http://lightquick.co.uk/instructions-for-the-panzer-network-gauge-xwidgets.html");
          }
}
//=====================
//End function
...
function menuitem12OnClick(Sender)
{
          ...
          {
              openURL("http://lightquick.co.uk/downloads/panzer-network-in-gauge-xwidget.html?Itemid=558");
          }
}
//=====================
//End function
...
function settooltip() {
    ...
    if ( preferencesHintPrefValue.text == "enabled" ) {
        ...
        greenLamp.hint = "The charging lamp";
        redLamp.hint = "The discharge lamp";
...
//======================================================================================
// Function to check whether there is spare capacity
//======================================================================================
function checkNetUsage() {
   //print("ramNetText.text "+ramNetText.text);

   var dd = ramNetText.text;
...
// reload if zero
//==============================
function zeroTimerOnUpdate(Sender)
{
 var totalAmount = parseInt(fieldNetInTotalK.text); 


 if (oldAmount == totalAmount ) {
     print("Running the zero timer - total amount in Kbytes " + totalAmount + " totalAmount");
     if (preferencesAutoRestartValue.text == "enabled" ) {
          //restart the widget
          widget.ForceToFround();
          wsc.SendKeys("{f5}");
     }      
...
// this function ets the mouse schroll wheel to resize one way or another
//=========================================================================
function button2OnMouseDown(Sender,Button,Shift,X,Y)
{
        if (preferencesAutoRestartValue.text == "enabled" ) {
               preferencesAutoRestartValue.text = "disabled" ;
        } else {
               preferencesAutoRestartValue.text = "enabled"  ;
